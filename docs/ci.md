  ...
  demojify:
    ...
    steps:
      ...
      - name: Audit for emoji
        run: >
          go run github.com/nicholashoule/demojify-sanitize/cmd/demojify@v1.0.0
          -root . -exts .go,.md,.txt,.yaml,.yml,.json
```

Two deliberate choices in that step:

- **Pin the version** (`@v1.0.0`, not `@latest`) so the gate's behavior only
...
```sh
go run github.com/nicholashoule/demojify-sanitize/cmd/demojify@v1.0.0 -root . -exts .go,.md
```
...
```sh
go run github.com/nicholashoule/demojify-sanitize/cmd/demojify@v1.0.0 -root . -json |
